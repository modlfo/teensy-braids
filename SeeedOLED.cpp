...
void SeeedOLED::displayOff(){
  sendCommand(SeeedOLED_Display_Off_Cmd);   //display off
}

void SeeedOLED::displayOn(){
  sendCommand(SeeedOLED_Display_On_Cmd);   //display off
}
...
void SeeedOLED::putChar(unsigned char C)
{
    ...
    unsigned char i=0;
    unsigned char CC = C-32;
    const unsigned char* KK = BasicFont[CC];
    for(i=0;i<8;i++)
    {
       //read bytes from code memory
       sendData(pgm_read_byte(&KK[i])); //font array starts at 0, ASCII starts at 32. Hence the translation
    }
}

void SeeedOLED::putCharLine(unsigned char C,unsigned char i)
{
    if(C < 32 || C > 127) //Ignore non-printable ASCII characters. This can be modified for multilingual font.
    {
    C=' '; //Space
